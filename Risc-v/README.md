...
### Test 2: Memory Access Instructions

This test validates the behavior of memory-related instructions such as `lw` and `sw`.

The objective is to verify that:

* Data can be written into data memory.
* Data can be read back from data memory.
* The correct value is written back into the register file.

Replace the image name with your simulation screenshot:

![Test 2 Simulation](test2_simulation.png)

---

### Test 3: Branch or Jump Instructions

This test validates the PC selection logic using branch or jump instructions.

The objective is to verify that:

* `PC + 4` is selected during normal sequential execution.
* The branch/jump target is selected when the control signal `PCSrc` is active.
* The processor updates the program counter correctly.

Replace the image name with your simulation screenshot:

![Test 3 Simulation](test3_simulation.png)
...
This project demonstrates the implementation of a simplified single-cycle RISC-V processor. The design integrates all major components of a basic CPU datapath, including instruction fetch, decode, execution, memory access, and write-back.

The modular implementation makes it easier to understand the role of each hardware block and how they interact to execute instructions. Simulation results confirm that the processor correctly executes basic RISC-V instructions and produces the expected output.
